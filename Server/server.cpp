...
void recv_msg(int idx) {
    ...
    while (1) {
        ...
        int recv_result = recv(sck_list[idx].sck, buf, MAX_SIZE, 0);
        ...
        if (recv_result > 0) {
            ...
            else if (flag.compare("/귓말") == 0)
            {
                ...
                pstmt = con->prepareStatement("INSERT INTO chatting(nick, time, recv) VALUE(?, NOW(),  ?)");
                pstmt->setString(1, sck_list[idx].user);
                pstmt->setString(2, whisper);
                pstmt->execute();

                pstmt = con->prepareStatement("SELECT nick, time, recv FROM chatting ORDER BY time DESC LIMIT 1");
